...
class RefMLPEncoder(BaseEncoder):
    def __init__(self, params):
        num_vars = params['num_vars']  # Debería ser 3 para tus datos de dimerización
        inp_size = params['input_size'] * params['input_time_steps']
        hidden_size = params['encoder_hidden']
        num_edges = params['num_edge_types']  # Debería ser 2 si solo hay 2 tipos de interacciones: dimerización y no dimerización
    ...
    def edge2node(self, edge_embeddings):
        incoming = torch.matmul(self.edge2node_mat, edge_embeddings)
        return incoming / (self.num_vars - 1)  # Normalización para el número de variables
